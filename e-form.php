...
if (!empty($resultEForm)) {
    ...
    foreach ($listArray as $key => $value) {
        ...
        <div class="container mb3">
            <h2 class="font-medium mb2"><?= $key ?></h2>
            <div class="row">
                <?php
                foreach ($value as $val) {
                    ...
                    <div class="col-md-4 col-xs-12 mb2">
                        <a href="<?= PDF_URL . $val['pdf_file'] ?>" download="<?= PDF_URL . $val['pdf_file'] ?>"
                           class="d-flex flex-column widget widget-img">
                            <div class="d-flex widget-image">
                                <?php
                                if(!empty($val['cover_image'])){
                                    $cover_image = $val['cover_image'];
                                    echo '<img src="'.FEATURE_PHOTO.$cover_image.'" class="center-block img-responsive">';
                                } else {
                                    echo '<img src="img/ebook.png" class="center-block img-responsive">';
                                }
                                ?>
                            </div>
                            <div class="widget-content">
                                <h5 class="margin_bottom_none font-medium font-size-16 file-title text-center"><?= $val['title'] ?></h5>
                            </div>
                        </a>
                    </div>
                    <?php
