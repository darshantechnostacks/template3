...
foreach ($blogs as $key => $value) {
    if (!in_array($value->id, $uBlogsStatusIds)) {
        ...
        if (isset($value->featured_image) && !empty($value->featured_image)) {
            $messg .= " <a href='blogdeatails.php?id=" . $value->id . "'><img src='" . API_URL . "geturl/uploads/feature_photo/" . $value->featured_image . "' class='img-responsive' style='max-height: 155px;min-height: 155px;width: 100%;'/></a>";
        } else {
            $messg .= " <a href='blogdeatails.php?id=" . $value->id . "'><img src='images/default-blog.jpg' class='img-responsive' style='max-height: 155px;min-height: 155px;width: 100%;'/></a>";
        }
        $dt = strtotime($value->created);
        $messg .= "</div>
                                    <div class='content-block'>
                                        <a href='blogdeatails.php?id=" . $value->id . "' class='title'>" . $value->title . "</a>";
        ...
        $messg .= $content;
        $messg .= "<a href='blogdeatails.php?id=" . $value->id . "' class='read-more'>Read more</a>
                                    </div>
                                </li>";
